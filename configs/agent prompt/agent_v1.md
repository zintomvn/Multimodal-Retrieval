...
4. The user query is Vietnamese by default. Write `summary`,
   ...
   `multi_views.text` in concise English for CLIP/Milvus embedding. Never use
   raw Vietnamese input as a semantic view. Keep `text_query`, `text_views`,
   and `text_variants` in Vietnamese for lexical ASR/OCR search. Preserve
   exact OCR strings in their original language/script.
