...
class RetrievalService:
    ...
    def _semantic_scores(
        ...
        for target in collections:
            ...
            search_video_ids = allowed_video_ids if allowed_video_ids is not None else dataset_video_ids
                ...
                try:
                    with stage("vector_batch"):
                        found = bounded_call(vector_batch, target.collection, [v for _,v in valid_vectors],
                            top_k, filters={"video_id": sorted(search_video_ids)})
            ...
            for variant_index, (variant, query_vector) in enumerate(zip(variants, query_vectors_by_model[model_id], strict=True)):
                ...
                try:
                    with stage("vector"):
                        hits = batches[variant_index] if batches is not None else bounded_call(
                            self.vector_client.search,
                            target.collection,
                            query_vector,
                            top_k=top_k,
                            filters={"video_id": sorted(search_video_ids)},
                        )
                except Exception:
                    backend_error = True
                    continue
    ...
    def _text_scores(
        ...
        if callable(batch):
            try:
                batch_results = bounded_call(batch, annotation_index(), [
                    ...
                    for source,variant,boosts in jobs], sorted(allowed_video_ids if allowed_video_ids is not None else dataset_video_ids))
            except Exception:
                return {}, True
        scores: dict[str, float] = {}
        backend_error = False
        for index,(expected_source,variant,boosts) in enumerate(jobs):
            ...
            for hit in hits:
                frame_id = self._resolve_keyframe_id(hit.id, hit.metadata)
                video_id = self._resolve_video_id(frame_id, hit.metadata) if frame_id else None
                if not frame_id or video_id not in dataset_video_ids:
                    continue
